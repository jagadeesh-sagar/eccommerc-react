...
function pseudoColor(str = '') {
  const p = ['#dbeafe','#fce7f3','#d1fae5','#fef3c7','#ede9fe','#fee2e2','#cffafe','#f3f4f6']
  let h = 0
...
function ProductsTab() {
  ...
  if (products.length === 0) return (
    <div className="text-center py-12 text-gray-500">
      ...
      <p className="text-sm mt-1">Switch to the <strong>Create Product</strong> tab to add one.</p>
    </div>
  )
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-400 mb-4">
        Showing all active products on the platform. Use <em>Create Product</em> to add yours.
      </p>
      {products.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">{p.brand_name} · {p.category_name}</p>
            </div>

            {/* Price */}
            <p className="text-sm font-bold text-[#B12704] flex-shrink-0">{fmt(p.base_price)}</p>
...
const EMPTY_VARIANT = { color: '', size: '', price: '', stock_qty: '', sku: '' }
        ...
        <div key={s.n} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <span className={[
              ...
              step === s.n ? 'text-[#c7511f]' : step > s.n ? 'text-green-600' : 'text-gray-400',
            ].join(' ')}>
              {s.label}
            </span>
          </div>
...
function ImageUploadStep({ productId, productName, onCreateAnother }) {
  ...
  const videoCount = uploads.filter(u => u.status === 'done' && u.isVideo).length
  const imgCount   = doneCount - videoCount

  const statusLabel = { queued: 'Waiting…', presign: 'Getting URL…', s3: 'Uploading to S3…', saving: 'Saving…', done: 'Done ✓', error: 'Failed' }
  const statusColor = { queued: 'text-gray-400', presign: 'text-blue-500', s3: 'text-blue-500', saving: 'text-amber-500', done: 'text-green-600', error: 'text-red-500' }
    ...
    <div className="max-w-2xl">
      ...
      >
        ...
        <p className="text-xs text-gray-400 text-center">
          Images: JPEG, PNG, WebP, GIF &nbsp;·&nbsp; Videos: MP4, MOV, WebM, MKV, AVI<br />
          ...
          {uploads.map(u => (
            <div
              ...
              className={[
                ...
                u.status === 'done'  ? 'border-green-200 bg-green-50'  :
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <span className={[
                    ...
                    u.isVideo ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700',
                  ].join(' ')}>
                    {u.isVideo ? 'video' : 'image'}
                  </span>
                </div>
                <p className={`text-xs ${statusColor[u.status] ?? 'text-gray-400'}`}>
                  {u.status === 'error' ? `⚠ ${u.error}` : statusLabel[u.status]}
                </p>
              </div>
      ...
      {doneCount > 0 && !busy && (
        <p className="mt-3 text-sm text-green-700 font-medium">
          ...
          {videoCount > 0 ? ` ${videoCount} video${videoCount !== 1 ? 's' : ''}` : ''} uploaded successfully.
        </p>
      )}
...
const EMPTY_FORM = {
  product_name: '', description: '', base_price: '',
  stock_qty: '', category: '', brand: '', sku: '', is_active: true,
}
...
function CreateProductTab() {
  ...
  useEffect(() => {
    ...
    client.get('/user/brand/').then(({ data }) => {
      setBrands(Array.isArray(data) ? data : (Array.isArray(data?.results) ? data.results : []))
    })
  }, [])
  ...
  function updateVariant(i, updated) { setVariants(v => v.map((r, idx) => idx === i ? updated : r)) }
  function removeVariant(i)          { setVariants(v => v.filter((_, idx) => idx !== i)) }

  async function handleSubmit(e) {
    e.preventDefault()
    const req  = ['product_name', 'base_price', 'stock_qty', 'category', 'brand']
    const errs = {}
    ...
    try {
      ...
      if (Object.keys(mapped).length) {
        ...
        setGlobalErr(d.detail || d.error || d.non_field_errors?.[0] || 'Could not create product.')
      }
    } finally {
      setSubmitting(false)
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <Field label="Product name" error={errors.product_name} required>
            <input className={inputCls(errors.product_name)} value={form.product_name}
              onChange={e => set('product_name', e.target.value)} placeholder="e.g. Samsung Galaxy S24" />
          </Field>
        </div>
        ...
        <Field label="Base price (₹)" error={errors.base_price} required>
          <input type="number" min="0" step="0.01" className={inputCls(errors.base_price)}
            value={form.base_price} onChange={e => set('base_price', e.target.value)} placeholder="0.00" />
        </Field>

        <Field label="Stock quantity" error={errors.stock_qty} required>
          <input type="number" min="0" className={inputCls(errors.stock_qty)}
            value={form.stock_qty} onChange={e => set('stock_qty', e.target.value)} placeholder="e.g. 100" />
        </Field>

        <Field label="SKU" error={errors.sku}>
          <input className={inputCls(errors.sku)} value={form.sku}
            onChange={e => set('sku', e.target.value)} placeholder="Unique stock-keeping unit" />
        </Field>
        ...
        <Field label="Brand" error={errors.brand} required>
          ...
          >
            ...
            {brands.map(b => (
              <option key={b.name ?? b.id} value={b.name}>{b.name}</option>
            ))}
          </select>
        </Field>
        ...
        <div className="sm:col-span-2">
          <label className="flex items-center gap-3 cursor-pointer select-none">
            ...
            <span className="text-sm font-medium text-gray-700">
              {form.is_active ? 'Active (visible to buyers)' : 'Inactive (hidden from buyers)'}
            </span>
          </label>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-gray-700">Variants <span className="text-gray-400 font-normal">(optional)</span></p>
        ...
        {variants.length === 0 && (
          <p className="text-xs text-gray-400 italic">
            No variants added. The product will use the base price and a single stock pool.
          </p>
        )}
      </div>
...
function BrandsTab() {
  ...
  async function handleSave(e) {
    e.preventDefault()
    if (!form.name.trim()) { setErrors({ name: 'Brand name is required.' }); return }
    ...
    try {
      ...
      const d = err.response?.data ?? {}
      if (d.name) { setErrors({ name: Array.isArray(d.name) ? d.name[0] : d.name }) }
      else { setGlobalErr(d.detail || d.error || 'Could not create brand.') }
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Brand name" error={errors.name} required>
              <input className={inputCls(errors.name)} value={form.name}
                onChange={e => setField('name', e.target.value)} placeholder="e.g. Samsung" />
            </Field>
            <Field label="Logo URL" error={errors.logo}>
              <input className={inputCls(errors.logo)} value={form.logo}
                onChange={e => setField('logo', e.target.value)} placeholder="https://…" />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Description" error={errors.description}>
                <textarea rows={2} className={inputCls(errors.description) + ' resize-none'}
                  value={form.description}
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500 italic">No brands yet. Add one above.</p>
      ) : (
        <div className="space-y-2">
          {brands.map((b, i) => (
            <div key={b.id ?? i}
            ...
            >
              {b.logo ? (
                <img src={b.logo} alt={b.name} className="w-10 h-10 object-contain rounded border border-gray-100" />
              ) : (
...
function AnswerBox({ question, qnaId, onAnswered }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!text.trim()) { setError('Answer cannot be empty.'); return }
    setSaving(true); setError('')
    try {
      await client.patch(`/user/product/seller-ans/${qnaId}`, { answer: text.trim() })
      setSaved(true)
      onAnswered?.()
    } catch (err) {
      const d = err.response?.data ?? {}
      setError(d.answer?.[0] || d.detail || d.error || 'Could not save answer.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-2 flex gap-2 items-start">
      ...
      <div className="flex flex-col gap-1.5 flex-shrink-0">
        ...
        {saved && !saving && !error && <p className="text-[10px] text-green-600 text-center">✓ Saved</p>}
      </div>
    </form>
  )
}
...
function QATab() {
  ...
  const loadQA = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(products) ? products : (Array.isArray(products?.results) ? products.results : [])
      ...
      const results = await Promise.allSettled(
        list.map(async (p) => {
          ...
          if (!pid) return null
          try {
            const { data: detail } = await client.get(`/user/product/detail/${pid}`)
  ...
  if (error) return (
    <div className="text-sm text-red-600">
      {error} <button onClick={loadQA} className="ml-2 underline">Retry</button>
    </div>
  )

  if (productQAs.length === 0) return (
    <div className="text-center py-16 text-gray-500">
      <p className="text-4xl mb-3">💬</p>
      <p className="font-medium text-gray-700">No customer questions yet.</p>
      <p className="text-sm mt-1">Questions from buyers will appear here once they start asking.</p>
    </div>
  )

  const totalUnanswered = productQAs.reduce(
    (s, pg) => s + pg.questions.filter(q => !q.answer).length, 0
  )
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {totalUnanswered > 0 && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-yellow-50 border border-yellow-300 rounded-lg text-sm">
          ...
          <span className="text-yellow-800 font-medium">
            {totalUnanswered} unanswered {totalUnanswered === 1 ? 'question' : 'questions'} need your attention.
          </span>
        </div>
      )}
      ...
      {productQAs.map((pg) => {
        ...
        return (
          <div key={pg.productId} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            {/* Product header */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400">
                    {pg.questions.length} {pg.questions.length === 1 ? 'question' : 'questions'}
                  </p>
                </div>
              </div>
              {unanswered > 0 && (
...
const ORDER_STATUS = {
  pending:    { label: 'Pending',    dot: 'bg-yellow-400', badge: 'bg-yellow-50 text-yellow-700 border-yellow-300' },
  processing: { label: 'Processing', dot: 'bg-blue-400',   badge: 'bg-blue-50 text-blue-700 border-blue-300'       },
  shipped:    { label: 'Shipped',    dot: 'bg-purple-400', badge: 'bg-purple-50 text-purple-700 border-purple-300' },
  delivered:  { label: 'Delivered',  dot: 'bg-green-500',  badge: 'bg-green-50 text-green-700 border-green-300'    },
  cancelled:  { label: 'Cancelled',  dot: 'bg-red-400',    badge: 'bg-red-50 text-red-700 border-red-300'          },
  returned:   { label: 'Returned',   dot: 'bg-orange-400', badge: 'bg-orange-50 text-orange-700 border-orange-300' },
}
    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
      ...
      <div className="bg-gray-50 border-b border-gray-200 px-5 py-3 flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-500">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400">Order date</p>
            <p className="text-gray-700 font-medium">{formatOrderDate(order.order_date)}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400">Buyer</p>
            <p className="text-gray-700 font-medium">{order.buyer_name ?? '—'}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400">Total</p>
            <p className="text-gray-700 font-medium">{fmt(order.total_amount)}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400">Items</p>
            <p className="text-gray-700 font-medium">{items.length}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {orderId && <span className="text-[11px] text-gray-400 font-mono">#{orderId}</span>}
      ...
      <div className="px-5 py-4">
        <div className="space-y-2 mb-3">
          {items.slice(0, expanded ? undefined : 2).map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-500 flex-shrink-0">Qty: <b>{item.quantity}</b></span>
            </div>
          ))}
        </div>
        ...
        {items.length > 2 && (
          ...
          >
            {expanded ? '▲ Show less' : `▼ Show ${items.length - 2} more item${items.length - 2 > 1 ? 's' : ''}`}
          </button>
        )}
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50 flex-shrink-0">
              <span className="text-sm font-semibold text-gray-800">💬 Chat with {order.buyer_name ?? 'Buyer'}</span>
              <button
                onClick={() => setChatOpen(false)}
      ...
      <div className="bg-gray-50 border-t border-gray-200 px-5 py-2.5 flex items-center justify-between">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5">
            <path fillRule="evenodd" d="M1 8.74c0 .983.713 1.825 1.69 1.943.764.092 1.534.163 2.31.21v2.607l3.918-2.606c.38.02.763.03 1.145.03 1.44 0 2.788-.233 3.985-.661.29-.104.549-.384.549-.743V5.26c0-.36-.26-.64-.55-.743A14.497 14.497 0 0 0 10.063 4H5.937A14.5 14.5 0 0 0 2.69 4.517C1.713 4.635 1 5.477 1 6.46v2.28Z" clipRule="evenodd" />
          </svg>
...
function SellerOrdersTab() {
  ...
  if (error) return (
    <div className="text-sm text-red-600">
      {error} <button onClick={fetchOrders} className="ml-2 underline">Retry</button>
    </div>
  )
      ...
      <main className="flex-1 max-w-screen-xl mx-auto w-full px-3 sm:px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Manage your products, brands, and customer Q&A</p>
          </div>
          <Link
            to="/"
            className="text-sm text-[#007185] hover:text-[#c7511f] hover:underline hidden sm:inline"
          >
            ← Back to store
          </Link>
        </div>
